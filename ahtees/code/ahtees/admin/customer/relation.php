...
if ($_REQUEST["newrelation"])
{
	unset($_SESSION["relationid"]);
}

if ($_REQUEST["relationid"])
{
	$_SESSION["relationid"] = $_REQUEST["relationid"];
}
...
if ($_POST["modifyrelation"])
{
	...
	$where = "id= ".$_SESSION["relationid"];
	modify_record("customer_relations", $data, $where);
}

if ($_REQUEST["deleterelation"])
{
...
if ($_SESSION["custid"]) 
{
	...
	$qry = "SELECT customer_relations.*, relation_master.description FROM customer_relations INNER JOIN relation_master ON (customer_relations.relation_id = relation_master.id) WHERE customer_id = ".$_SESSION["custid"] . " order by relation_master.description";
	$crelations = mysqli_query($connDB, $qry) or die('Query failed: ' . mysqli_error($connDB)); 
	$crelation = mysqli_fetch_assoc($crelations);
	
	if ($_SESSION["relationid"])
	{
		$qry = "SELECT customer_relations.* FROM customer_relations WHERE customer_relations.id = ".$_SESSION["relationid"];
		//echo $qry."<BR>";
...
<table width ="100%" cellpadding="0" cellspacing="0" border="1">
	<tr>
		<td width="50%" valign="top" align="left">
			...
			<table cellpadding="0" cellspacing="1">
				<tr>
					...
					<td>
						<select name="relation_id">
							...
							<?php do { ?>
							<option value ="<?php echo $relation["id"]; ?>" <?php if ($mrelation["relation_id"] == $relation["id"]) {?>selected<?php } ?>><?php echo $relation["description"]; ?></option>
				...
				<tr>
					<td>&nbsp;</td>
					<?php if ($_SESSION["relationid"]) { ?>
						<td colspan="3"><input type="submit" value="Modify Relations"><br /><input type="button" value="New relation" onclick="javascript:location.href='admin_add_customer.php?newrelation=1';"/></td>				
					<?php } else { ?>
		...
		<?php if ($crelation) {?>
		<div style="overflow:auto; height:540px; width:660px"> 
			<table width="100%">
				<tr bgcolor="#999999">
				...
				<tr bgcolor="<?php echo $bgcolor; ?>">
					...
					<td><a href="admin_add_customer.php?relationid=<?php echo $crelation["id"]; ?>">edit</a>&nbsp;<a href="admin_add_customer.php?deleterelation=<?php echo $crelation["id"]; ?>">delete</a></td>
				</tr>
				<?php 
				if ($bgcolor == "WHITE")
				{
