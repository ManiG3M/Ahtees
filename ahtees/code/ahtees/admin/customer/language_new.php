...
if (isset($_POST["modifycuslanguage"]))
{
	...
	$where = "id= ".$_REQUEST["clangid"];
	modify_record("customer_language", $data, $where);
}

if (isset($_REQUEST["deletelang"]))
{
...
if ($_REQUEST["custid"]) 
{
	...
	$qry = "SELECT customer_language.*, language_master.description FROM customer_language INNER JOIN language_master ON (customer_language.lang_id = language_master.id) WHERE customer_id = ".$_REQUEST["custid"];
	$clangs = mysqli_query($connDB, $qry) or die('Query failed: ' . mysqli_error($connDB)); 
	$clang = mysqli_fetch_assoc($clangs);
	
	if (isset($_REQUEST["clangid"]))
	{
		$qry = "SELECT customer_language.* FROM customer_language WHERE customer_language.id = ".$_REQUEST["clangid"];
		//echo $qry."<BR>";
...
?>
<h1>Editing Language of <?php echo $_REQUEST["display_name"]; ?> </h1>
		...
		<td width="50%" valign="top">
			...
			<table cellpadding="0" cellspacing="1" border="0">
				<tr>
					...
					<td>
						<select name="lang_id">
							...
							<?php if (isset($mlanguage["lang_id"])&&($mlanguage["lang_id"] == $lang["id"])) {?>
							selected
							<?php } ?>>
							<?php echo $lang["description"]; ?></option>
					...
					<td><select name="fluency_level">
							<option value="0">Select one...</option>
							<option value="1" <?php if (isset($mlanguage["fluency_level"])&&($mlanguage["fluency_level"] == 1)) {?>
							selected
							<?php } ?>>One</option>
							<option value="2" <?php if (isset($mlanguage["fluency_level"])&&($mlanguage["fluency_level"] == 2)) {?>
							selected
							<?php } ?>>Two</option>
							<option value="3" <?php if (isset($mlanguage["fluency_level"])&&($mlanguage["fluency_level"] == 3)) {?>
							selected
							<?php } ?>>Three</option>
							<option value="4" <?php if (isset($mlanguage["fluency_level"])&&($mlanguage["fluency_level"] == 4)) {?>
							selected
							<?php } ?>>Four</option>
							<option value="5" <?php if (isset($mlanguage["fluency_level"])&&($mlanguage["fluency_level"] == 5)) {?>
							selected
							<?php } ?>>Five</option>
						</select></td>
				</tr>	
				<tr>
					<td>&nbsp;</td>
					<?php if (isset($_REQUEST["clangid"])) { ?>
						<td colspan="3"><input type="submit" value="Modify Language">&nbsp;&nbsp;
						<input type="button" value="New Language" onclick="javascript:location.href='language_new.php?newlang=1&custid=<?php echo $_REQUEST["custid"]; ?>&userid=<?php echo $_REQUEST["userid"]; ?>&display_name=<?php echo $_REQUEST["display_name"]; ?>';"/></td>				
					<?php } else { ?>
		...
		<?php if (isset($clang)) {?>
			<table width="100%">
				...
				<tr bgcolor="<?php echo $bgcolor; ?>">
					...
					<td><a href="language_new.php?clangid=<?php echo $clang["id"]; ?>&custid=<?php echo $_REQUEST["custid"]; ?>&userid=<?php echo $_REQUEST["userid"]; ?>&display_name=<?php echo $_REQUEST["display_name"]; ?>">edit</a>&nbsp;<a href="language_new.php?deletelang=<?php echo $clang["id"]; ?>&custid=<?php echo $_REQUEST["custid"]; ?>&userid=<?php echo $_REQUEST["userid"]; ?>&display_name=<?php echo $_REQUEST["display_name"]; ?>">delete</a></td>
				</tr>
				<?php 
				if ($bgcolor == "WHITE")
				{
